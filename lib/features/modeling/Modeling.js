...
import ReplaceRootHandler from './cmd/ReplaceRootHandler';
...
Modeling.prototype.getHandlers = function() {
  return {
    ...
    'root.replace': ReplaceRootHandler
  };
};
...
/**
 * Update the root element of the canvas by setting it to a new root element.
 * Note that this does not move child elements of the existing root.
 *
 * @param {Object} context
 * @param {djs.model.Root} context.newRoot the new root object
 *
 * @return {djs.model.Root} the new root object
 */
Modeling.prototype.updateRoot = function(newRoot) {
  var context = {
    newRoot: newRoot
  };
  ...
  return context.newRoot;
};

/**
 * Replace the canvas root object by: (1) creating a new root object, (2) moving
 * from an old root object to new root object, and (3) replacing the old canvas root
 * object by the newly created root object.
 *
 * @param {djs.model.Root} oldRoot
 * @param {Object | djs.model.Root} newRoot
 *
 * @return {djs.model.Root} the new root object
 */
Modeling.prototype.replaceRoot = function(oldRoot, newRoot) {
  newRoot = this._create('root', newRoot);
  ...
  this._commandStack.execute('root.replace', context);
