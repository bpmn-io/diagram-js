...
export default function SpaceTool(
  ...
  eventBus.on('spaceTool.getAffectedShapes', LOW_PRIORITY, function(context) {
    var axis = context.axis,
        delta = context.delta,
        start = context.start;

    return self._getAffectedShapes(axis, delta, start);
  });
...
SpaceTool.prototype.init = function(event, context) {
  ...
  var affectedShapes = this._eventBus.fire('spaceTool.getAffectedShapes', {
    axis: axis,
    delta: delta,
    start: start
  });

  if (!affectedShapes.movingShapes) {
    affectedShapes.movingShapes = [];
  }

  if (!affectedShapes.resizingShapes) {
    affectedShapes.resizingShapes = [];
  }

  var minDimensions = this._eventBus.fire('spaceTool.getMinDimensions', {
    axis: axis,
    direction: direction,
    shapes: affectedShapes.resizingShapes,
    start: start
  });

  var spaceToolConstraints = getSpaceToolConstraints(affectedShapes, axis, direction, start, minDimensions);
...
/**
 * Get shapes to be moved and resized.
 *
 ...
 * @return {Object}
 */
SpaceTool.prototype._getAffectedShapes = function(axis, delta, start) {
  var rules = this._rules;

  var movingShapes = this._elementRegistry.filter(function(element) {
    return !isRoot(element)
      && !isConnection(element)
      && isShapeMoving(element, axis, delta, start);
  });

  var resizingShapes = this._elementRegistry.filter(function(element) {
    return !isRoot(element)
      && !isConnection(element)
      && isShapeResizing(element, axis, start)
...
export function isShapeMoving(shape, axis, delta, start) {
  var shapeStart = shape[ axis ],
      shapeEnd = shapeStart + shape[ AXIS_TO_DIMENSION[ axis ] ];

  return (delta > 0 && shapeStart > start) || (delta < 0 && shapeEnd < start);
}

export function isShapeResizing(shape, axis, start) {
  var shapeStart = shape[ axis ],
      shapeEnd = shapeStart + shape[ AXIS_TO_DIMENSION[ axis ] ];

  return shapeStart < start && shapeEnd > start;
}