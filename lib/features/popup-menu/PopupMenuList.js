 ...
 * @param {() => void} props.onPointerInteraction
...
export default function PopupMenuList(props) {
  ...
  const handleMouseMove = (event) => {
    ...
    const entryEl = domClosest(event.target, '.entry', true);

    if (!entryEl || !resultsRef.current.contains(entryEl)) {
      return;
    }
    ...
    for (const group of groupedEntries) {
      const entry = group.entries.find(entry => String(entry.id) === entryId);
