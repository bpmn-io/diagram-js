...
function matchItem(item, words, keys) {
  ...
  const {
    ...
    let tokens, matchedWords;

    if (isArray(itemValue)) {
      ({ tokens, matchedWords } = itemValue.reduce(
        (result, itemString, i) => {
      ...
      ({ tokens, matchedWords } = matchString(itemValue, words));
    }
...
function scoreTokens(tokens) {
  ...
  // use max
  // return Math.max(...tokens.map((token) => scoreToken(token)));
}
...
function scoreToken(token) {
  ...
  if (isArray(token)) {
    return token.reduce((sum, token) => sum + scoreToken(token), 0);
  }
  ...
  score = (
    token.start
      ? (
        token.end ? 131.9 : 7.87
      )
      : (
        token.wordStart ? 2.19 : 1
      )
  ) * modifier;
...
function matchString(string, words) {
  ...
  while ((match = regexp.exec(string))) {
    ...
    const start = startIndex === 0; // start of words
    const end = endIndex === string.length; // end of words
