...
function applyConstraints(attr, trbl, resizeConstraints) {
  ...
  if (isNumber(minValue)) {
    value = (/top|left/.test(attr) ? Math.min : Math.max)(value, minValue);
  }

  if (isNumber(maxValue)) {
    value = (/top|left/.test(attr) ? Math.max : Math.min)(value, maxValue);
  }
...
export function getMinResizeBounds(direction, currentBounds, minDimensions, childrenBounds) {
  ...
  var combinedBox = {
    top: Math.min(minBox.top, childrenBox.top),
    left: Math.min(minBox.left, childrenBox.left),
    bottom: Math.max(minBox.bottom, childrenBox.bottom),
    right: Math.max(minBox.right, childrenBox.right)
  };
