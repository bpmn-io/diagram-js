...
var VIEWBOX_THROTTLE_INTERVAL = 300;
...
export default function KeepSelectionVisibleOnResize(eventBus, canvas, selection) {
  ...
  // canvas.viewbox.changed may fire many times in quick succession
  // so throttle the recomputation
  eventBus.on('canvas.viewbox.changed', throttle(() => {
    this._selectionWasVisible = isSelectionVisible();
  }, VIEWBOX_THROTTLE_INTERVAL));
  ...
  eventBus.on('canvas.resized', () => {
    ...
    canvas.scrollToElement(selected, SCROLL_PADDING);
  });
