...
export default function Dragging(eventBus, canvas, selection, elementRegistry) {
  ...
  function move(event, activate) {
    ...
    if (!context.active && (activate || getLength(globalDelta) > context.threshold)) {
      ...
      assign(payload, {
        x: Math.round(localStart.x + displacement.x),
        y: Math.round(localStart.y + displacement.y),
        dx: 0,
        dy: 0
      }, { originalEvent: event });
    ...
    if (context.active) {
      ...
      assign(payload, {
        x: Math.round(localCurrent.x + displacement.x),
        y: Math.round(localCurrent.y + displacement.y),
        dx: Math.round(localDelta.x),
        dy: Math.round(localDelta.y)
      }, { originalEvent: event });
