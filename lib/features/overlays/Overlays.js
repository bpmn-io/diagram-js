...
Overlays.prototype._updateOverlayContainer = function(container) {
  ...
  if (container.topRight) {
    setPosition(container.topRight, width, 0);
  }

  if (container.bottomRight) {
    setPosition(container.bottomRight, width, height);
  }

  if (container.bottomLeft) {
    setPosition(container.bottomLeft, 0, height);
  }
...
Overlays.prototype._addOverlay = function(overlay) {
  ...
  htmlContainer = domify('<div class="djs-overlay" data-overlay-id="' + id + '" style="order: ' + order + ';">');
  ...
  if (isString(overlay.position)) {
    ...
    case 'top-left':
      container = overlayContainer.topLeft;

      if (!container) {
        container = domify('<div class="djs-overlays-relative djs-overlays-top-left" style="position: absolute;"></div>');

        overlayContainer.html.appendChild(container);
        overlayContainer.topLeft = container;
      }

      container.appendChild(htmlContainer);
      break;
    case 'top-right':
      container = overlayContainer.topRight;

      if (!container) {
        container = domify('<div class="djs-overlays-relative djs-overlays-top-right" style="position: absolute;"></div>');

        overlayContainer.html.appendChild(container);
        overlayContainer.topRight = container;
      }

      container.appendChild(htmlContainer);
      break;
    case 'bottom-left':
      container = overlayContainer.bottomLeft;

      if (!container) {
        container = domify('<div class="djs-overlays-relative djs-overlays-bottom-left" style="position: absolute;"></div>');

        overlayContainer.html.appendChild(container);
        overlayContainer.bottomLeft = container;
      }

      container.appendChild(htmlContainer);
      break;
    case 'bottom-right':
      container = overlayContainer.bottomRight;

      if (!container) {
        container = domify('<div class="djs-overlays-relative djs-overlays-bottom-right" style="position: absolute;"></div>');

        overlayContainer.html.appendChild(container);
        overlayContainer.bottomRight = container;
      }
    ...
    this._updateOverlayContainer(overlayContainer);
  } else {
