...
ContextPad.prototype._updateAndOpen = function(target) {
  ...
  forEach(orderEntries(entries), function(entry, id) {
    var grouping = entry.group || DEFAULT_GROUP_NAME,
...
/**
 * Order entries by `groupOrder` first and by `order` second. Both are optional
 * and default to 0. For `groupOrder`, the first entry of a group with a defined
 * `groupOrder` will define the order of the group.
 *
 * @param {ContextPadEntries} entries
 *
...
function orderEntries(entries) {
  ...
  const entriesGroupedByGroup = groupBy(entries, (entry) => entry.group || DEFAULT_GROUP_NAME);

  // 3. order groups by groupOrder
  const groupsOrdered = sortBy(entriesGroupedByGroup, (entries) => {
    const entry = entries.find((entry) => isDefined(entry.groupOrder));

    if (entry) {
      return entry.groupOrder;
    }

    return 0;
  });

  // 4. order entries by order
  for (const group in groupsOrdered) {
    groupsOrdered[group] = sortBy(groupsOrdered[group], (entry) => {
      return entry.order || 0;
    });
  }

  // 5. remove ID from entries and return
  return groupsOrdered.reduce((entries, group) => {
