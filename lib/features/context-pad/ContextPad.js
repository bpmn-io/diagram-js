...
ContextPad.prototype._init = function() {
  ...
  this._eventBus.on('drag.init', function(event) {
    if (self.isOpen(event.element)) {
      self.hide();
    }
  });

  this._eventBus.on('drag.ended', function(event) {
    if (self.isOpen(event.element)) {
      self.show();
    }
  });
...
ContextPad.prototype.isHidden = function() {
  return this.isOpen() && !domClasses(this._current.html).has('open');
};
