...
Canvas.prototype._setRoot = function(rootElement, layer) {
  ...
  if (rootElement) {
    ...
    svgAppend(this._viewport, layer);

    var elementsToUpdate = [rootElement];

    for (var i = 0; i < elementsToUpdate.length; i++) {
      var e = elementsToUpdate[i];

      console.log(e);

      e.children && elementsToUpdate.push(...e.children);

      if (!e.parent) {
        continue;
      }
      try {
        var gfx = self._graphicsFactory.create(e._type, e, e._parentIndex);

        self._elementRegistry.updateGraphics(e, gfx);

        self._graphicsFactory.update(e._type, e, gfx);
      } catch (err) {
        debugger;
      }
    }

    // selfAndAllChildren(rootElement).forEach(function(e) {
    //   var gfx = self._graphicsFactory.create(e._type, e, e._parentIndex);

    //   self._elementRegistry.updateGraphics(rootElement, gfx);
...
Canvas.prototype._addElement = function(type, element, parent, parentIndex) {
  ...
  element._type = type;
  element._parentIndex = parentIndex;

  var gfx;
  if (this.findRoot(element) === this.getRootElement()) {

    // create graphics
    gfx = graphicsFactory.create(type, element, parentIndex);
    ...
    gfx = graphicsFactory.create('placeholder', element, parentIndex);
