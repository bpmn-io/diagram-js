 ...
 *
 * When passed a single element (or its ID), the canvas navigates to it,
 * switching to the element's root if necessary. When passed a collection
 * of elements, the viewbox is scrolled to their combined bounding box
 * within the current root without switching roots; the elements are
 * expected to be on the current root.
 *
 * @param {ShapeLike|ConnectionLike|string|Array<ShapeLike|ConnectionLike>} element The element(s) to scroll to or the ID of a single element.
...
Canvas.prototype.scrollToElement = function(element, padding) {
  ...
  // accept a single element or a collection of elements
  ...
  // a single element is navigated to (switching root if necessary); a
  // collection is framed within the current root (see method documentation)
  if (!isArray(element)) {
  ...
  const elementBounds = getBoundingBox(elements),
