 ...
 */
export default function Canvas(config, eventBus, graphicsFactory, elementRegistry, scheduler) {
...
Canvas.prototype._setRoot = function(rootElement, layer) {
  ...
  this._eventBus.fire('root.set.start', { element: rootElement });
  ...
  selfAndAllChildren(rootElement).forEach(function(e) {
    var promise = scheduler.changePriority(e.id, Infinity);
...
Canvas.prototype._addElement = function(type, element, parent, parentIndex) {
  ...
  graphicsFactory.update(type, element, gfx).then(() => {
    eventBus.fire(type + '.rendered', { element: element, gfx: gfx });
  });

  eventBus.fire(type + '.added', { element: element });
